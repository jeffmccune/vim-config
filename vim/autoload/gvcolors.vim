...
"          mosh at http://www.cs.albany.edu/~mosh             - original idea
" Date:    Oct 08, 2008
...
" user override/specification on where rgb.txt is
if exists("g:gvcolors_rgbtxtfile")
 if filereadable(g:gvcolors_rgbtxtfile)
  let s:rgbtxtfile= g:gvcolors_rgbtxtfile
 else
  echohl WarningMsg | echo "Unable to find ".g:gvcolors_rgbtxtfile."!" | echohl None
 endif
endif

" default tests which look for rgb.txt
...
"  call Decho("unable to find rgbtxtfile")
  echohl WarningMsg | echo "Unable to find rgb.txt!  Suggest setting g:gvcolors_rgbtxtfile to its path" | echohl None
 endif
endif
...
" ---------------------------------------------------------------------
" s:DisplayColors: display a page of colors {{{1
fun! s:DisplayColors(linechg)
"  call Dfunc("s:DisplayColors(linechg=".a:linechg.") g:topline=".g:topline.(exists("do_rgb_stl")? " do_rgb_stl=".s:do_rgb_stl : " <null do_rgb_stl>"))
...
"   call Decho("chg=[curline=".curline."]-[topline=".g:topline."]=".s:chg)
  ...
  if line(".") > g:firstline
   if s:chg != 0
    call s:DisplayColors(s:chg)
   endif
  else
   let g:topline=g:firstline
   call s:DisplayColors(0)
  endif
...
nn   <buffer> <silent> j             :call <SID>DisplayColors(1)<cr>
nn   <buffer> <silent> k             :call <SID>DisplayColors(-1)<cr>
nn   <buffer> <silent> <c-d>         :call <SID>DisplayColors(&scroll)<cr>
nn   <buffer> <silent> <c-u>         :call <SID>DisplayColors(-&scroll)<cr>
nn   <buffer> <silent> <c-f>         :call <SID>DisplayColors(g:linesperpage)<cr>
nn   <buffer> <silent> <c-b>         :call <SID>DisplayColors(-g:linesperpage)<cr>
nn   <buffer> <silent> G             :call <SID>DisplayColors(line("$") - g:linesperpage + 1 - g:topline)<cr>
nn   <buffer> <silent> gg            :let g:topline= g:firstline<cr>:call <SID>DisplayColors(0)<cr>
nn   <buffer> <silent> <c-l>         H0:let g:topline= line(".")<cr>:call <SID>DisplayColors(0)<cr>
